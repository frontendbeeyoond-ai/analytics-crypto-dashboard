...
import { DocumentsAnalyticsData } from "@/types/analytics";
...
function KPICard({ label, value, highlight }: { label: string; value: string | number; highlight?: boolean }) {
  return (
    <div className="rounded-xl p-6" style={highlight ? { background: "#1e1b4b", border: "1px solid #3730a3" } : CARD}>
      <p className="text-xs font-medium uppercase tracking-wider" style={{ color: highlight ? "#c7d2fe" : "#6b7280" }}>{label}</p>
...
function EventsOverTimeChart({ data }: { data: { date: string; value: number }[] }) {
  ...
  return (
    <div className="rounded-xl p-6" style={CARD}>
      <h3 className="text-sm font-semibold uppercase tracking-wider text-white">Document Clicks Over Time</h3>
      ...
      <div className="mt-4 h-52">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={formatted} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            ...
            <Tooltip contentStyle={TT} labelFormatter={(l) => `Date: ${l}`} formatter={(v: number) => [v.toLocaleString(), "Document Clicks"]} />
            ...
            {rows.map((row) => {
              ...
              return (
                <tr key={row.dimension} onClick={() => setSelected(isSelected ? null : row.dimension)} title={truncate ? row.dimension : undefined}
                  ...
                  style={{ borderBottom: "1px solid #1f2937", background: isSelected ? "rgba(30,27,75,0.5)" : undefined }}
                  onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "rgba(255,255,255,0.03)"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = isSelected ? "rgba(30,27,75,0.5)" : ""; }}
                >
                  <td className="px-5 py-3 font-medium text-white">
                    <div className="flex items-center gap-2">
                      {isSelected && <span className="h-2 w-2 shrink-0 rounded-full bg-indigo-400" />}
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <div className="h-1.5 w-16 overflow-hidden rounded-full" style={{ background: "#1f2937" }}>
                        <div className="h-full rounded-full bg-indigo-500" style={{ width: `${Math.min(parseFloat(share), 100)}%` }} />
                      </div>
...
function DocumentsAnalyticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/analytics/documents${qs ? `?${qs}` : ""}`, { cache: "no-store" });
        if (!res.ok) { const b = await res.json().catch(() => ({})); throw new Error(b.details || b.error || "Failed to fetch documents analytics"); }
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2.5">
                <div className="rounded-lg p-2" style={{ background: "#1e1b4b", color: "#818cf8" }}>
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">documents_click</h1>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {isLoading && !data && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="mt-4 text-sm" style={{ color: "#4b5563" }}>Loading documents analytics…</p>
          </div>
        )}
        {data && (
          <>
            ...
            <section className="mb-8"><RealtimeSection eventName="documents_click" accentColor="indigo" /></section>
            <section className="mb-8 grid gap-6 lg:grid-cols-2">
              <BreakdownTable title="Click Location" dimensionLabel="Location" rows={data.clickLocationBreakdown} totalEvents={data.totalEvents} />
              <BreakdownTable title="Destination Page" dimensionLabel="Page" rows={data.destinationPageBreakdown} totalEvents={data.totalEvents} truncate />
            </section>
            <section className="mb-8">
              <BreakdownTable title="CTA Name" dimensionLabel="CTA Name" rows={data.ctaNameBreakdown} totalEvents={data.totalEvents} />
            </section>
          </>
        )}
      </main>
...
export default function DocumentsPage() {
  return <Suspense><DocumentsAnalyticsPage /></Suspense>;
}
