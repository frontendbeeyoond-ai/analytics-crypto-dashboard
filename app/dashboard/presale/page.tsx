...
function KPICard({ label, value, highlight }: { label: string; value: string | number; highlight?: boolean }) {
  return (
    <div className="rounded-xl p-6" style={highlight ? { background: "#14532d", border: "1px solid #166534" } : CARD}>
      <p className="text-xs font-medium uppercase tracking-wider" style={{ color: highlight ? "#bbf7d0" : "#6b7280" }}>{label}</p>
...
function EventsOverTimeChart({ data }: { data: { date: string; value: number }[] }) {
  ...
  return (
    <div className="rounded-xl p-6" style={CARD}>
      <h3 className="text-sm font-semibold uppercase tracking-wider text-white">Presale Clicks Over Time</h3>
      ...
      <div className="mt-4 h-52">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={formatted} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            ...
            <Tooltip contentStyle={TT} labelFormatter={(l) => `Date: ${l}`} formatter={(v: number) => [v.toLocaleString(), "Presale Clicks"]} />
            ...
            {rows.map((row) => {
              ...
              return (
                <tr key={row.dimension} onClick={() => setSelected(isSelected ? null : row.dimension)} title={truncate ? row.dimension : undefined}
                  ...
                  style={{ borderBottom: "1px solid #1f2937", background: isSelected ? "rgba(5,46,22,0.5)" : undefined }}
                  onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "rgba(255,255,255,0.03)"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = isSelected ? "rgba(5,46,22,0.5)" : ""; }}
                >
                  <td className="px-5 py-3 font-medium text-white">
                    <div className="flex items-center gap-2">
                      {isSelected && <span className="h-2 w-2 shrink-0 rounded-full bg-green-400" />}
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <div className="h-1.5 w-16 overflow-hidden rounded-full" style={{ background: "#1f2937" }}>
                        <div className="h-full rounded-full bg-green-500" style={{ width: `${Math.min(parseFloat(share), 100)}%` }} />
                      </div>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/analytics/presale${qs ? `?${qs}` : ""}`, { cache: "no-store" });
        if (!res.ok) { const b = await res.json().catch(() => ({})); throw new Error(b.details || b.error || "Failed to fetch presale analytics"); }
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2.5">
                <div className="rounded-lg p-2" style={{ background: "#052e16", color: "#34d399" }}>
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 15l-2 5L9 9l11 4-5 2z" /></svg>
                </div>
                <div>
                  <h1 className="text-xl font-bold text-white">presale_click_cta</h1>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {isLoading && !data && (
          <div className="flex flex-col items-center justify-center py-24">
            <div className="h-10 w-10 animate-spin rounded-full border-4 border-green-900 border-t-green-500" />
            <p className="mt-4 text-sm" style={{ color: "#4b5563" }}>Loading presale analytics…</p>
          </div>
        )}
        {data && (
          <>
            ...
            <section className="mb-8"><RealtimeSection eventName="presale_click_cta" accentColor="green" /></section>
            <section className="mb-8 grid gap-6 lg:grid-cols-2">
              ...
              <BreakdownTable title="CTA Location" dimensionLabel="Location" rows={data.ctaLocationBreakdown} totalEvents={data.totalEvents} />
            </section>
            <section className="mb-8 grid gap-6 lg:grid-cols-2">
            ...
            <section className="mb-8">
              <BreakdownTable title="Presale Destination URL" dimensionLabel="URL" rows={data.presaleDestinationUrlBreakdown} totalEvents={data.totalEvents} truncate />
            </section>
          </>
        )}
      </main>
...
export default function PresalePage() {
  return <Suspense><PresaleAnalyticsPage /></Suspense>;
}
